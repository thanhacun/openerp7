...
class kderp_asset_depreciation(osv.osv): 
    ...
    def init(self,cr):
        cr.execute("""Create or replace view  kderp_asset_depreciation as
                        Select
                        ...
                        from
                            ...
                            where 
                                koe.state not in ('cancel','draft') and coalesce(asset_id,0)>0
                            Union
                            select
                                kge.code as expense,
                                kge.date,
                                kgec.code || '-' || kgec.description as job,
                                kgebc.code || ' - ' || kgebc.name as budget,
                                kgel.amount,
                                kgel.asset_id  
                            from 
                                kderp_general_expense_line kgel
                            left join 
                                kderp_general_expense kge on kgel.general_expense_id = kge.id
                            left join
                                kderp_general_expense_code kgec on general_expense_code_id=kgec.id
                            left join
                                kderp_general_expense_budget_code kgebc on budget_id=kgebc.id
                            where 
                                kge.state not in ('cancel','draft') and kge.type in ('others','monthly-expense') and coalesce(asset_id,0)>0) vwcombine""")       
