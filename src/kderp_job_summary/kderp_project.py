                        ...
                        if kcp.state not in ('cancel','draft'):
                            ...
                            for kpfc_line in kcp.invoice_line:
                                ...
                                if kpfc_line.account_analytic_id.id==kp.id:
                                    ...
                                    Claimed_amount_CTC+=cur_obj.round(cr, uid, company_currency, exrate*subtotal) #Claimed Amount in Compnay Currency
...
#JOB RECEIVED VAT    
    ...
    def _get_job_received_info(self, cr, uid, ids, name, arg, context=None):
        ...
        cr.execute("""Select 
                    ...
                    from
                        (select distinct account_analytic_id,supplier_payment_id from kderp_supplier_payment_line kspl where account_analytic_id in (%s)) kspl
                    left join
                    ...
                    left join
                        ...
                        left join
                            ...
                            where
                                ...
                                and ksp.id in (Select distinct supplier_payment_id from kderp_supplier_payment_line where account_analytic_id in (%s))
                        ...
                        where ksp.currency_id<>(Select currency_id from res_company rc limit 1) and ksp.id in (Select distinct supplier_payment_id from kderp_supplier_payment_line where account_analytic_id in (%s))) vwrate on ksp.id=vwrate.ksp_id
                    left join
                        purchase_order po on order_id=po.id
                    left join
                        (Select 
                        ...
                        left join
                            ...
                            from 
                                purchase_order_line pol
                            where
                                order_id in (Select order_id from purchase_order_line where account_analytic_id in (%s))
                            group by
                                order_id) vwgrant_total on pol.order_id=vwgrant_total.order_id
                        where
                            pol.order_id in (Select order_id from purchase_order_line where account_analytic_id in (%s))
                        group by
                            pol.order_id,
                    ...
                    where
                        ...
                        where 
                            ...
                            ksp.state not in ('draft','cancel') and po.state not in ('draft','cancel') and
                            kspl.account_analytic_id in (%s)
                    Group by
                        kspl.account_analytic_id""" % (job_ids,job_ids,job_ids,job_ids,job_ids,job_ids))
        #raise osv.except_osv("E",cr.fetchall())
        for id in ids:
        ...
        cr.execute("""Select 
                    ...
                    left join
                        ...
                        left join
                            ...
                            where
                                kspe.id in (Select distinct supplier_payment_expense_id from kderp_supplier_payment_expense_line where account_analytic_id in (%s)) and
                    ...
                    left join
                        ...
                        left join
                            ...
                            from 
                                kderp_other_expense_line koel
                            where
                                koel.expense_id in (Select distinct expense_id from kderp_other_expense_line where account_analytic_id in (%s))
                            group by
                    ...
                    where
                        ...
                        kspe.state not in ('draft','cancel') and koe.state not in ('draft','cancel') and
                        kspel.account_analytic_id in (%s)
                    Group by
                        kspel.account_analytic_id""" % (job_ids,job_ids,job_ids,job_ids,job_ids))
        for job_id,vat_amount,vat_subtotal in cr.fetchall():
            res[job_id]['vat_received_amount']+=vat_amount or 0
            res[job_id]['vat_received_subtotal']+=vat_subtotal or 0
        return res
    ...
    _columns={             
              ...
              'vat_received_amount':fields.function(_get_job_received_info,type='float',digits_compute=dp.get_precision('Budget'), method=True,string='VAT Received',multi='_get_received_vat',
                                                 store={
                                                        ...
                                                        'kderp.supplier.vat.invoice':(_get_job_received_from_supplier_vat,['kderp_supplier_payment_ids','kderp_supplier_payment_expense_ids'],35),
                                                        'kderp.expense.budget.line':(_get_job_from_job_expense_budget_line,None,50),
                                                        'account.analytic.account':(lambda self, cr, uid, ids, c={}: ids, ['need_update'], 50),
                                                      }),
