...
-- select 
-- * 
-- from dblink('dbname=KDVN_Data_HCM user=openerp password=admin host=192.168.1.11','Select id,parent_id,sequence,type,cat_code,name from budget_category') as bc(id int,parent_id int,sequence int,type varchar(16),cat_code varchar(32),name varchar(64))
...
-- select * from dblink('dbname=KDVN_Data_HCM user=openerp password=admin host=192.168.1.11',
...
-- Select max(id) from account_budget_post;-- 820
-- alter SEQUENCE account_budget_post_id_seq RESTART with 821
...
-- -- 
-- --delete from kderp_brand_name
-- Insert into kderp_brand_name (id,code,name) select * 
-- from 
-- dblink('dbname=KDVN_Data_HCM user=openerp password=admin host=192.168.1.11',
-- 'Select id,code,name from kdvn_brand_name') as kbn(id int,code varchar(10),name varchar(50))
-- Select max(id) from kderp_brand_name
-- alter SEQUENCE kderp_brand_name_id_seq RESTART with 25


--### Code Japan###
-- delete from kderp_product_category_japan
-- select * 
-- from 
-- dblink('dbname=KDVN_Data_HCM user=openerp password=admin host=192.168.1.11',
...
-- dblink('dbname=KDVN_Data_HCM user=openerp password=admin host=192.168.1.11',
-- 'Select id,code,name from kdvn_product_category_japan') as kbn(id int,code varchar(4),name varchar(255))
-- Select max(id) from kderp_product_category_japan 86
-- alter SEQUENCE kderp_product_category_japan_id_seq RESTART with 87
...
-- ---- Select max(id) from product_template; 64598
-- alter SEQUENCE product_template_id_seq RESTART with 64599
...
-- Select max(id) from product_product; 71865
--alter SEQUENCE product_product_id_seq RESTART with 71866
...
-- Select max(id) from res_partner ; --5412
-- alter SEQUENCE res_partner_id_seq RESTART with 5413
...
-- 
-- Delete from mail_alias where id not in (1,2,3)
...
-- 	dblink('dbname=KDVN_Data_HCM user=openerp password=admin host=192.168.1.11',
-- 	'Select id,login from res_users where id<>1') as ru(id int,login varchar(128))
-- Select max(id) from mail_alias ; 1037
-- alter SEQUENCE mail_alias_id_seq RESTART with 1038
...
-- Select max(id) from res_users ; 604
-- alter SEQUENCE res_users_id_seq RESTART with 605