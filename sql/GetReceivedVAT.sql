...
left join
	...
	where
		...
		and po.id in (Select distinct order_id from purchase_order_line where account_analytic_id in (849))
	group by
...
left join
	...
	left join
		...
		purchase_order_line pol
	    where
		order_id in (Select distinct order_id from purchase_order_line where account_analytic_id in (849))
	    group by
		order_id) vwgrant_total on pol.order_id=vwgrant_total.order_id
	where
	    pol.order_id in (Select distinct order_id from purchase_order_line where account_analytic_id in (849)) and 
...
where                        
	vwtemp_job_per.account_analytic_id in (849)
Group by
	vwtemp_job_per.account_analytic_id
Union
Select 
...
left join
	...
	where
		...
		and 
		koe.id in (Select distinct expense_id from kderp_other_expense_line where account_analytic_id in (849))
	group by
...
left join
	...
	left join
		...
		kderp_other_expense_line koel
	    where
		expense_id in (Select distinct expense_id from kderp_other_expense_line where account_analytic_id in (849))
	    group by
		expense_id) vwgrant_total on koel.expense_id = vwgrant_total.expense_id
	where
	    koel.expense_id in (Select distinct expense_id from kderp_other_expense_line where account_analytic_id in (849)) and 
