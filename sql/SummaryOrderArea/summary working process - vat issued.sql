CREATE OR REPLACE FUNCTION funSummaryWorkingProcess(getDate date)
...
Select
		...
		sum(other_internal_support)::float as other_internal_support
from
	(Select 
		job_type,
	...
	left join
		...
		group by
		    contract_id) astemp on  kcja.contract_id = astemp.contract_id 
	left join
	...
	left join
		...
		where
			to_char(ki.date,'Mon-YY')=to_char(getDate,'Mon-YY')
		group by
