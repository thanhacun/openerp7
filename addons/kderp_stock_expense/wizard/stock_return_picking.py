...
class stock_return_picking(osv.osv_memory):
    ...
    _columns = {
        ...
        'invoice_state': fields.selection([('2binvoiced', 'To be refunded/invoiced'), ('none', 'No invoicing')], 'Invoicing'),
    }
        ...
        if record_id:
            ...
            for m  in pick.move_lines:
                if m.state == 'done' and m.product_qty * m.product_uom.factor > return_history.get(m.id, 0):
                    valid_lines += 1
            if not valid_lines:
                raise osv.except_osv(_('Warning!'), _("No products to return, may be this packing already returned !"))
        return res
...
#        Create new picking for returned products
        ...
        new_pick_name = self.pool.get('ir.sequence').get(cr, uid, seq_obj_name + ".%s" % new_type)
        old_pick = pick_obj.browse(cr, uid, pick.id)
        note = (old_pick.note + "\n" if old_pick.note else "")  + "Return packing from old Number %s" % old_pick.name
        new_picking = pick_obj.copy(cr, uid, pick.id, {
                                        'name': _('%s') % (new_pick_name),
                                        'move_lines': [],
                                        'note': note,
                                        'origin':pick.name,
                                        'state':'draft',
                                        'location_id': old_pick.location_dest_id.id,
                                        'location_dest_id': old_pick.location_id.id,
                                        'type': new_type,
                                        'date':date_cur,
        ...
        for v in val_id:
            ...
            if new_qty:
                ...
                new_move=move_obj.copy(cr, uid, move.id, {
                                            ...
                                            'from_analytic_id': move.to_analytic_id and move.to_analytic_id.id,
                                            'to_analytic_id': move.from_analytic_id and move.from_analytic_id.id,
                                            'date': date_cur,
        ...
        context['picking_id'] = new_picking
        model_list = {
                'out': 'stock.picking',
                'in': 'stock.picking',
